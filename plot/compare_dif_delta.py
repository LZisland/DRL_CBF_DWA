...
# 读取表格数据
file_path1 = "cbf_dwa_0.5_1.xlsx"
file_path2 = "cbf_dwa_0.5_1.5.xlsx"
file_path3 = "cbf_dwa_0.5_2.xlsx"
file_path4 = "cbf_dwa_0.5_2.5.xlsx"
file_path5 = "cbf_dwa_0.5_3.xlsx"
...
obstacles = [
    {"center": [5, 5, 5], "radius": 3,   "alpha": 0.2},
    {"center": [5, 5, 5], "radius": 2.5, "alpha": 0.2},
    {"center": [5, 5, 5], "radius": 2,   "alpha": 0.2},
    {"center": [5, 5, 5], "radius": 1.5, "alpha": 0.2},
    {"center": [5, 5, 5], "radius": 1,   "alpha": 0.2},
]
...
draw_arrow_with_cone(ax, start=[5 + 3.5*np.sqrt(2)/2, 5 - 3.5*np.sqrt(2)/2, 5], end=[5+1/np.sqrt(2)+0.02, 5-1/np.sqrt(2)-0.02, 5],
                     cone_length=0.3, cone_radius=0.1, color='black')
# 添加标签
ax.text(5 + 3.6*np.sqrt(2)/2, 5 - 3.6*np.sqrt(2)/2, 5, r"$\delta=1$", fontsize=12, fontfamily='Times New Roman')
draw_arrow_with_cone(ax, start=[9, 5, 5], end=[8.25, 5, 5],
                     cone_length=0.3, cone_radius=0.1, color='black')
# 添加标签
ax.text(9.1, 5.1, 5, r"$\delta=3$", fontsize=12, fontfamily='Times New Roman')

# 起点终点标记
ax.scatter(*start, marker="*", s=80, c="black")
...
# 保存图片（高分辨率）
plt.savefig("dif_delta_compare_plot.pdf", dpi=300, pad_inches=0.2)
