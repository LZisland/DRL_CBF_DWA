...
file_path2 = "cbf_dwa_0.5_2_[ 8.5 11.5 10. ].xlsx"
file_path3 = "cbf_dwa_0.5_2_[ 9.5 10.5 10. ].xlsx"
file_path4 = "cbf_dwa_0.5_2_[ 8 12 10].xlsx"
file_path5 = "cbf_dwa_0.5_2_[ 9 11 10].xlsx"
...
draw_arrow_with_cone(ax, start=[5 + 3.5*np.sqrt(2)/2, 5 - 3.5*np.sqrt(2)/2, 5], end=[5+np.sqrt(2)+0.02, 5-np.sqrt(2)-0.02, 5],
                     cone_length=0.3, cone_radius=0.1, color='black')
# 添加标签
ax.text(5 + 3.6*np.sqrt(2)/2, 5 - 3.6*np.sqrt(2)/2, 5, r"$\delta=2$", fontsize=12, fontfamily='Times New Roman')



# 起点终点标记
ax.scatter(*start, marker="o", s=60, c="black")
ax.scatter(*end1, marker="^", s=80, c="red")
ax.scatter(*end2, marker="^", s=80, c="red")
ax.scatter(*end3, marker="^", s=80, c="red")
ax.scatter(*end4, marker="^", s=80, c="red")
...
# 保存图片（高分辨率）
plt.savefig("dif_dest_compare_plot.pdf", dpi=300)
