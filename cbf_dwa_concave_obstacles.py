...
def remove_duplicate_spheres(spheres, tol=1e-8):
    ...
    for center, radius in spheres:
        duplicated = False
        for old_center, old_radius in unique:
            if np.linalg.norm(center - old_center) < tol and abs(radius - old_radius) < tol:
                duplicated = True
                break
        if not duplicated:
...
def generate_u_shape_spheres(segments, half_width, half_height, safety_factor=1.03):
    left_segment, right_segment, bottom_segment = segments
    rho = np.sqrt(half_width ** 2 + half_height ** 2)
    spheres = []
    ...
    for start, end in (left_segment, right_segment):
        ...
        span = length / 4.0
        radius = safety_factor * np.sqrt(rho ** 2 + span ** 2)
        for s_i in (0.25 * length, 0.75 * length):
            spheres.append((start + direction * s_i, radius))

    start, end = bottom_segment
    start = np.asarray(start, dtype=float)
    end = np.asarray(end, dtype=float)
    length = np.linalg.norm(end - start)
    direction = (end - start) / length
    span = length / 8.0
    radius = safety_factor * np.sqrt(rho ** 2 + span ** 2)
    for s_i in np.linspace(0.125 * length, 0.875 * length, 4):
        spheres.append((start + direction * s_i, radius))

    return spheres


def build_concave_obstacles():
    half_width = 0.28
    half_height = 0.22
    segment_sets = [
        [
            (np.array([2.8, 3.2, 3.6]), np.array([2.8, 4.9, 3.6])),
            (np.array([5.2, 3.2, 3.6]), np.array([5.2, 4.9, 3.6])),
            (np.array([2.8, 3.2, 3.6]), np.array([5.2, 3.2, 3.6])),
        ],
        [
            (np.array([5.6, 6.4, 7.4]), np.array([5.6, 8.0, 7.4])),
            (np.array([8.0, 6.4, 7.4]), np.array([8.0, 8.0, 7.4])),
            (np.array([5.6, 6.4, 7.4]), np.array([8.0, 6.4, 7.4])),
        ],
        [
            (np.array([7.0, 7.8, 8.8]), np.array([7.0, 9.1, 8.8])),
            (np.array([9.2, 7.8, 8.8]), np.array([9.2, 9.1, 8.8])),
            (np.array([7.0, 7.8, 8.8]), np.array([9.2, 7.8, 8.8])),
        ],
    ]

    spheres = []
    for segments in segment_sets:
        spheres.extend(generate_u_shape_spheres(segments, half_width, half_height))
    spheres = remove_duplicate_spheres(spheres)
    obstacle_array = np.array([[c[0], c[1], c[2], r] for c, r in spheres], dtype=float)
    return segment_sets, obstacle_array
...
def simulate():
    ...
    segment_sets, obstacles = build_concave_obstacles()
    ...
    print("Scenario: concave obstacles in the long-range CBF-DWA environment")
    ...
    save_trajectory_history(
        trajectory,
        planner.dt,
        "concave_uav_trajectory.xlsx",
    )
    print("Distance data saved to: concave_obstacle_distances.xlsx")
    print("Trajectory data saved to: concave_uav_trajectory.xlsx")
